...
class Fond(BaseModel):
    ...
    def __init__(self, districtId,price):
        super().__init__()
        self.districtId = districtId
        self.price = price
        
    def __str__(self):
        return f'{self.id},{self.districtId},{self.price}'
    
    def print():
        for item in BaseModel.objects(Fond.table):
            print(item)
    ...
    def districtId(self,districtId):
        with shelve.open(getAbsolutePath(District.table)) as db:
            if str(districtId) in db:
                self.__districtId = districtId
            else:
                self.__districtId = 0
                self.isValid = True
                raise Exception("o`quvchi topilmadi !!!")
            
    @property
    def brand(self):
        with shelve.open(getAbsolutePath(District.table)) as db:
            return db[str(self.districtId)]
